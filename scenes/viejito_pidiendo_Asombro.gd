...
func _on_viejito_dialog():
	if not asked_oldman:
		...
		yield(dialog("little_chef", "..."), "done")
		yield(dialog("viejito", "Es extraño pensar en mi hogar. Tuve varios a lo largo de los años, pero el que más se afianzó en mi memoria fue el primero luego de nuestro arribo."), "done")	
